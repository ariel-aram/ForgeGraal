...
exports.default = new forgescript_1.NativeFunction({
    ...
    execute(ctx, [path, target]) {
        try {
            const info = BinaryInspector_1.BinaryInspector.inspect((0, functions_1.resolveFileArg)(ctx, path));
            if (!info)
                return this.success(false);
            if (!target)
                return this.success(true);
            return this.success(BinaryInspector_1.BinaryInspector.matchesTarget(info, (0, functions_1.requireTarget)(target).id));
        }
        catch (err) {
