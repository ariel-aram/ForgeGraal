...
Not provided, and said so when used: the post-quantum key types (ML-KEM, ML-DSA, SLH-DSA) and `SubtleCrypto.supports` in `crypto`. AES-OCB (`createCipheriv`), cSHAKE, TurboSHAKE, KT128/KT256 and KMAC (Web Crypto; KMAC keys import and export as JWK only, as in Node) run in JavaScript over the host's AES and Keccak. BLAKE2b512 and BLAKE2s256 (hash, HMAC, PBKDF2, HKDF) and Argon2 (`crypto.argon2`, `argon2Sync`, and Web Crypto Argon2d/i/id) are pure JavaScript on the host, so they run at interpreter speed: about 0.4 ms per Argon2 memory block and 1 to 2 MB/s of BLAKE2 input; `createSign`/`createVerify` with a BLAKE2 digest are not provided. `cluster` runs on `fork()`: workers, their events and messages, and several workers listening on one port.
