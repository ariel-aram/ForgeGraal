...
| `dns`, `dns/promises` | `Resolver`, `getServers`, `setServers`, `lookup`, `lookupService`, and `resolve`, `resolve4`, `resolve6`, `resolveCname`, `resolveMx`, `resolveNs`, `resolveTxt`, `resolveSrv`, `resolvePtr`, `resolveSoa`, `resolveCaa`, `resolveNaptr`, `reverse` over UDP with TCP fallback, with system resolver discovery (/etc/resolv.conf on POSIX, ipconfig /all on Windows). |
...
| `test`, `test/reporters` | `node:test` runner: `test`, `it`, `describe`, `suite`, `before`, `after`, `beforeEach`, `afterEach`, `run`, `.skip`, `.todo`, `.only`, subtests, diagnostics, `mock` (`fn`, `method`, `timers`), and Spec, TAP, Dot, JUnit and LCOV reporters matching Node.js 26.9.0's format. |
