...
| `node:test`, `node:test/reporters` | Reachable only under the `node:` scheme, as in Node. `test`/`it`, `describe`/`suite`, the four hooks, subtests, `skip`/`todo`/`only`/`expectFailure`, `t.plan`, `t.assert`, `t.diagnostic`, `t.signal`, `t.waitFor`, timeouts, callback tests, `mock` (`fn`, `method`, `getter`, `setter`, `property`, `timers`), `run()`, the default report and exit code, and the spec, TAP, dot, JUnit and lcov reporters, checked against Node.js 24 and 26. A test after an `await` must be started with `t.test()` (the engine has no async context to find its parent by); `run()` runs files in-process (`isolation: 'none'`); coverage, snapshots and watch mode throw rather than pretend; `mock.module` is absent, as in Node without `--experimental-test-module-mocks`. |
...
Not provided, and said so when used: `crypto` has no constant-time guarantee in its JavaScript. AES-OCB (`createCipheriv`), cSHAKE, TurboSHAKE, KT128/KT256 and KMAC (Web Crypto; KMAC keys import and export as JWK only, as in Node) run in JavaScript over the host's AES and Keccak. BLAKE2b512 and BLAKE2s256 (hash, HMAC, PBKDF2, HKDF) and Argon2 (`crypto.argon2`, `argon2Sync`, and Web Crypto Argon2d/i/id) are pure JavaScript on the host, so they run at interpreter speed: about 0.4 ms per Argon2 memory block and 1 to 2 MB/s of BLAKE2 input; `createSign`/`createVerify` with a BLAKE2 digest are not provided. ML-KEM, ML-DSA and SLH-DSA (`generateKeyPair`, sign and verify, `encapsulate`/`decapsulate`, and Web Crypto ML-KEM and ML-DSA with `SubtleCrypto.supports`) are provided. The post-quantum code is JavaScript, not C: ML-KEM and ML-DSA take milliseconds, SLH-DSA signing takes about a second for the `f` sets and 10 to 25 seconds for the `s` sets (verification is fast). Where Node.js 24 and 26 differ (PKCS#8 export of ML-KEM and ML-DSA keys, expanded-only keys, some error texts), the host follows 24: PKCS#8 carries the seed and the expanded key, and both forms, the seed alone and the expanded key alone, are read. Like Node, SLH-DSA is not in Web Crypto; signing is hedged (random), as OpenSSL's default. The JavaScript is not constant-time, and an expanded-only ML-KEM key is checked for canonical encoding and its stored hash, not for the consistency of its two halves. `cluster` runs on `fork()`: workers, their events and messages, and several workers listening on one port.
