...
test("PURE_JS_FORGEDB_DRIVERS lists exactly the non-native drivers", () => {
	...
	for (const [driver, spec] of Object.entries(FORGEDB_DRIVERS)) {
		assert.equal(
			PURE_JS_FORGEDB_DRIVERS.includes(driver as never),
			!spec.native,
		);
	}
});
...
test("suggestAlternative recommends a pure JS driver only for native ones", () => {
	assert.equal(
		ForgeDBIntegration.suggestAlternative("sqlite"),
		PURE_JS_FORGEDB_DRIVERS[0],
	);
	assert.equal(
		ForgeDBIntegration.suggestAlternative("better-sqlite3"),
		PURE_JS_FORGEDB_DRIVERS[0],
	);
