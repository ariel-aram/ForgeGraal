...
test("Every file in dist/functions is loadable by ForgeScript's FunctionManager", () => {
	const files = readdirSync(functionsDir, { recursive: true }).filter((f) =>
	...
	assert.equal(
		names.length,
		files.length,
		"each function file must default-export a NativeFunction",
	);
	...
	for (const fn of FunctionManager.toJSON()) {
		if (fn.args?.length) {
			...
			assert.equal(
				fn.brackets,
				firstOptional !== 0,
				`${fn.name}: brackets must match required args`,
			);
		}
	}
});
