...
export class V8AddonBuilder {
	...
	public static build(options: {
		...
		if (!toolchain) {
			throw new RuntimeError(
				`${pkg.name} is a native addon compiled against V8, and ForgeGraal builds those from source with the ` +
					`target's own C++ toolchain, which is not wired up for ${meta.name} yet.`
			);
		}
		const gypFile = join(pkg.packageDir, "binding.gyp");
		...
		const work = mkdtempSync(join(tmpdir(), "forgegraal-v8addon-"));
		try {
			...
			const compileTarget = (t: GypTargetSettings, isMain: boolean) => {
				...
				for (const source of t.sources) {
					...
					if (res.status !== 0) {
						throw new RuntimeError(
							`Compiling ${relative(pkg.packageDir, source)} of ${pkg.name} for ${meta.name} failed:\n${(res.stderr || res.stdout).trim().split("\n").slice(0, 25).join("\n")}`
						);
					}
					objects.push(object);
				}
	...
	private static expand(value: string, vars: Vars, packageDir: string, gypDir: string): string {
		...
		out = out.replace(/<\(([A-Za-z_]+)\)/g, (_m, name: string) => {
			if (name === "module_root_dir") return packageDir;
			if (name === "DEPTH") return gypDir;
			if (name in vars) return vars[name];
			throw new RuntimeError(
				`binding.gyp uses the variable '<(${name})', which ForgeGraal's gyp reader does not define.`
			);
		});
		if (/<!?\(/.test(out)) {
			throw new RuntimeError(`binding.gyp uses a command expansion ForgeGraal's gyp reader does not support: ${value}`);
		}
		return out;
	}
	...
	private static resolveDependency(spec: string, packageDir: string, vars: Vars): GypTargetSettings {
		...
		const gypFile = resolve(packageDir, file);
