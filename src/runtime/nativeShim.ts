 ...
 */
export const OPTIONAL_ACCELERATORS = [
	"zlib-sync",
	"msgpackr-extract",
	"pg-native",
	"mediaplex",
 ...
 */
export const UNSUBSTITUTABLE_NATIVE = [
	"lmdb",
	"canvas",
	"@napi-rs/canvas",
	"@gifsx/gifsx",
...
export function createNativeShimSource(config: NativeShimConfig): string {
	...
	var OPTIONAL_ACCELERATORS = ${JSON.stringify(OPTIONAL_ACCELERATORS)};
	var UNSUBSTITUTABLE = ${JSON.stringify(UNSUBSTITUTABLE_NATIVE)};
 ...
 */
export const WASM_FALLBACKS_SOURCE: string = createNativeShimSource({
	target: "unknown",
});
