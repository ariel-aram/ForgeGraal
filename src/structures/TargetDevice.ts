...
const WIN7_PINNED_WARNING =
	"Using Node.js 12.22.12, the version community reports converge on as the last that actually launches " +
 ...
 */
const VISTA_PINNED_WARNING =
	"Using Node.js 5.12.0, the last official release that still runs on Windows Vista (Node 6.0.0 dropped " +
...
const VISTA_PINNED_NODE_X86: PinnedLegacyNode = {
	version: "5.12.0",
	fileKey: "win-x86-exe",
	warning: VISTA_PINNED_WARNING,
};
const VISTA_PINNED_NODE_X64: PinnedLegacyNode = {
	version: "5.12.0",
	fileKey: "win-x64-exe",
	warning: VISTA_PINNED_WARNING,
};
...
export const TARGET_METADATA_MAP: Readonly<Record<TargetDevice, Readonly<TargetMetadata>>> = {
	...
	[TargetDevice.WinVistaX86]: {
		...
		description:
			"Win32 console executable for NT 6.0 only — Node.js 6.0.0 dropped Vista, so this is a separate, older pin from Windows 7",
		arch: "x86",
		bits: 32,
		os: "windows-legacy",
		is32BitOrLegacy: true,
		...
		bootstrapInstall: null,
		runtimeHint:
			"Node.js 5.12.0 (the last official release that runs on Vista at all) is downloaded and verified " +
	...
	[TargetDevice.WinVistaX64]: {
		...
		description:
			"Win64 console executable for NT 6.0 only — Node.js 6.0.0 dropped Vista, so this is a separate, older pin from Windows 7",
		arch: "x64",
		bits: 64,
		os: "windows-legacy",
		is32BitOrLegacy: true,
	...
	[TargetDevice.WinLegacyX86]: {
		...
		bootstrapInstall: null,
		runtimeHint:
			"Node.js 12.22.12 (community-reported as the version that actually launches on real Windows 7 " +
