...
export default new NativeFunction({
	...
	version: "1.0.0",
	description:
		"Returns whether ForgeGraal builds a target for the given or detected package manager",
	unwrap: true,
	brackets: true,
	...
	execute(ctx, [target, pm]) {
		try {
			return this.success(
				PolicyEnforcer.checkTarget(target, packageManagerArg(ctx, pm)).allowed,
			);
		} catch (err) {
